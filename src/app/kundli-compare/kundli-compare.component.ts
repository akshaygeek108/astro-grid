...
@Component({
  selector: 'kundli-compare',
  templateUrl: './kundli-compare.component.html',
  styleUrls: ['./kundli-compare.component.css']
})
export class KundliCompareComponent {
  @ViewChild('kundliContainer1') container1!: ElementRef;
  @ViewChild('kundliContainer2') container2!: ElementRef;  // Not static - created dynamically
  centerLabels: Record<number, string> = {
    1: '',
    2: ''
  };
  kundliTitle1 = 'Kundli 1';
  kundliTitle2 = 'Kundli 2';
  chart1Type = '';
  chart2Type = '';
  planets = [
  ...
  // Add second kundli
  addKundli(chartType: string, kundliTitle: string) {
    if (!this.kundli2Enabled) {
      this.kundli2Enabled = true;
      this.chart2Type = chartType;
      this.kundliTitle2 = kundliTitle;
      setTimeout(() => this.drawBase(2, chartType), 0);
    }
  }

  addKundli1(chartType: string, kundliTitle: string) {
    if (!this.kundli1Enabled) {
      this.kundli1Enabled = true;
      this.chart1Type = chartType;
      this.kundliTitle1 = kundliTitle;
      setTimeout(() => this.drawBase(1, chartType), 0);
    }
  }
  ...
  deleteKundli1() {
    ...
    if (this.container1) {
      this.centerLabels[1] = '';
      d3.select(this.container1.nativeElement).selectAll('*').remove();
    }
  }
  ...
  resetGrid() {
    ...
    this.drawBase(1, this.chart1Type);
    if (this.kundli2Enabled) {
      this.drawBase(2, this.chart2Type);
    }
  }

  // Draw base for a specific kundli
  drawBase(kundliId: number = 1, chartType?: string) {
    ...
    } else if (chartType === 'southIndianChartKundli') {
    ...
    } else if (chartType === 'diamondChartKundli') {
    ...
    svg.on('dblclick', (event: MouseEvent) => {
      ...
      if (userText !== null) {
        ...
        this.drawBase(kundliId, kundliId === 1 ? this.chart1Type : this.chart2Type);                 // 🔥 redraw ONLY that chart
      }
    });
    const houseZones = [
  ...
  onDrop(event: DragEvent, houseId: number, dropX?: number, dropY?: number, kundliId: number = 1,) {
    ...
    this.drawBase(kundliId, kundliId === 1 ? this.chart1Type : this.chart2Type);
  }
}
